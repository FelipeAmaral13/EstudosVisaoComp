...
def count_dolar():
    ...
    '''
    This function has the objective of detecting, pointing out and counting the amount of coins.
    ...
    # Read image
    img_dolar_coin = cv2.imread('Images\\dolar_original.png')

    # Verify if can read the image
    if not img_dolar_coin is None:
    ...
    imag_erode = cv2.morphologyEx(
        imgdilate, cv2.MORPH_OPEN, struct_element2, iterations=4)
    ...
    im_with_keypoints = cv2.drawKeypoints(imgdilate, keypts, np.array(
        []), (0, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    res = cv2.drawKeypoints(imgdilate, keypts, np.array(
        []), (0, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    ...
    cv2.imwrite(path + '\\image_result\\dolar_result.jpg', img_dolar_coin)
